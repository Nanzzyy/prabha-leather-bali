...
supabase/product-seo.sql
supabase/promo-campaigns.sql
```
...
Apply `supabase/product-seo.sql` to add optional per-product Meta title and Meta description fields. They are editable from Products → Edit product → SEO metadata and are emitted server-side on each product page; blank fields fall back to the product title and description.

Apply `supabase/promo-campaigns.sql` to enable the Promotions workspace at `/admin/promos/`. Admins can create or edit a named campaign, choose existing catalog products, and assign a discounted price to each item. The separate Website navigation setting controls whether the promotion appears in the public navbar. When enabled, the selected active campaign name becomes the navbar label and its slug becomes the public URL (`/<slug>/`); when disabled, the link and public promotion page are unavailable. Re-run this additive script after pulling updates so existing installations receive the menu settings table, RLS policies, and save functions.
