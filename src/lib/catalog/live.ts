...
const SELECT = 'id, title, slug, description, leather_type, base_price_usd, is_featured, categories(slug), product_images(image_url, display_order), product_variants(sku, color_name, color_hex, size_eu, stock_status)';
...
export function mapLiveProductRow(row: any): Product {
  ...
  return {
    ...
    images: (row.product_images || []).sort((a: any, b: any) => a.display_order - b.display_order).map((i: any) => i.image_url),
    variants: (row.product_variants || []).map((v: any) => ({
      sku: v.sku,
      color: v.color_name,
      colorHex: v.color_hex || undefined,
      size: v.size_eu || undefined,
      priceAdjustment: 0,
...
export async function fetchLiveLooks(): Promise<LiveLook[] | null> {
  ...
  try {
    return await dedup(lookHolder, LIVE_CACHE_TTL, async () => {
      const { data, error } = await supabase!.from('looks').select(`id, title, image_url, image_url_2, display_order, is_active, look_spots(id, x, y, image_index, display_order, products(${SELECT}))`).eq('is_active', true).order('display_order');
      if (error || !data || !data.length) return null;
      return data.map((row: any) => ({
        id: row.id,
        image: row.image_url,
        images: [row.image_url, row.image_url_2].filter(Boolean),
        title: row.title,
        displayOrder: Number(row.display_order ?? 0),
        spots: (row.look_spots ?? [])
          .sort((a: any, b: any) => a.display_order - b.display_order)
          .filter((s: any) => s.products)
          .map((s: any) => ({ product: mapLiveProductRow(s.products), x: Number(s.x), y: Number(s.y), imageIndex: Number(s.image_index ?? 0) })),
      }));
    });
  } catch {
    return null;
