...
export default function AdminPromosPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [nextCampaigns, nextProducts] = await Promise.all([listPromoCampaigns(), listProducts()]);
      setCampaigns(nextCampaigns);
      setProducts(nextProducts);
      setSchemaMissing(false);
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Promotions could not be loaded.';
      setSchemaMissing(/promo_campaign|promo_item|schema cache|does not exist|404/i.test(message));
      err(message);
    } finally {
      setLoading(false);
    }
      ...
      {loading ? <div className="admin-loading"><Icon>progress_activity</Icon><span>Loading promotions…</span></div> : <div className="admin-promo-workspace">
        ...
        <form className="admin-promo-editor admin-card" onSubmit={save}>
          ...
          <div className="admin-fieldrow">
            ...
            <label className="admin-checkbox" style={{ alignSelf: 'center' }}><input type="checkbox" checked={draft.is_active} onChange={(event) => setDraft((current) => ({ ...current, is_active: event.target.checked }))} /><span>Show this promotion in the website menu</span></label>
          </div>
          ...
          <div className="admin-sticky-actions"><span className="admin-field__hint"><Icon>info</Icon> Only active promotions appear on the public website.</span><button type="submit" className="admin-btn admin-btn--dark" disabled={saving || schemaMissing}><Icon>save</Icon>{saving ? 'Saving…' : 'Save promotion'}</button></div>
        </form>
      </div>}
