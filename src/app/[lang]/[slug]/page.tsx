...
export default async function CustomPromoPage({ params }: { params: Promise<{ lang: string; slug: string }> }) {
  const { slug } = await params;
  const campaign = await getLivePromoCampaignBySlug(slug);
  if (!campaign) notFound();

  return <main className="promo-page promo-page--detail"><header className="promo-hero"><LocaleLink className="promo-back" href="/catalog/"><Icon>arrow_back</Icon> Back to collection</LocaleLink><span className="eyebrow">Special selection</span><h1>{campaign.name}</h1>{campaign.description && <p>{campaign.description}</p>}</header><section className="promo-detail-grid">{campaign.items.map((item) => <ProductCard key={item.id} product={item.product} priceOverride={item.promoPrice} promoLabel="Promo" />)}</section></main>;
}
